//! Date.prototype.getFullYear — observable builtin algorithm.

use ts2wasm_spec_kernel::algorithm::{AlgoBuilder, SpecAlgoProgram};
use ts2wasm_spec_kernel::SpecOp;

pub fn build_date_get_full_year() -> SpecAlgoProgram {
...
mod tests {
    ...
    #[test]
    fn algorithm_builds() {
        let program = build_date_get_full_year();
        let trace = predict_trace(&program);
        assert!(!trace.is_empty(), "algorithm should produce trace");
    }
    #[test]
    fn date_get_full_year_has_call_specop() {
        let program = build_date_get_full_year();
        let trace = predict_trace(&program);
        assert!(trace.iter().any(|e| e.kind == "CallSpecOp"), "must call SpecOps");
    }
}
