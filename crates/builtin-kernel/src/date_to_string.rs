//! Date.prototype.toString — observable builtin algorithm.
//! Returns a string representation of the Date.

use ts2wasm_spec_kernel::algorithm::{AlgoBuilder, SpecAlgoProgram};
use ts2wasm_spec_kernel::SpecOp;

pub fn build_date_to_string() -> SpecAlgoProgram {
...
mod tests {
    ...
    #[test]
    fn algorithm_builds() { let p = build_date_to_string(); assert!(!predict_trace(&p).is_empty()); }
    #[test]
    fn date_to_string_has_call_specop() { let p = build_date_to_string(); assert!(predict_trace(&p).iter().any(|e| e.kind == "CallSpecOp")); }
}
