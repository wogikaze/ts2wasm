//! Date.prototype.toUTCString — observable builtin algorithm.
//! Returns UTC date/time string representation.

use ts2wasm_spec_kernel::algorithm::{AlgoBuilder, SpecAlgoProgram};
use ts2wasm_spec_kernel::SpecOp;

pub fn build_date_to_utc_string() -> SpecAlgoProgram {
...
mod tests {
    ...
    #[test]
    fn algorithm_builds() { let p = build_date_to_utc_string(); assert!(!predict_trace(&p).is_empty()); }
    #[test]
    fn date_to_utc_string_has_call_specop() { let p = build_date_to_utc_string(); assert!(predict_trace(&p).iter().any(|e| e.kind == "CallSpecOp")); }
}
