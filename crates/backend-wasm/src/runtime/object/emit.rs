...
"#,
    ...
    pub(crate) fn emit_object_from_entries(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $object_from_entries (param $iterable i32) (result i32)
    (local $tag i32)
    (local $base i32)
    (local $len i32)
    (local $i i32)
    ...
    (local $obj i32)
    (local $entry_tag i32)
    (local.set $tag (i32.and (local.get $iterable) (i32.const {tag_mask})))
          ...
          (i32.load
            (i32.add
              ...
              (i32.add (i32.const {array_header})
                (i32.shl (local.get $i) (i32.const {elem_shift}))))))
        (local.set $entry_tag (i32.and (local.get $elem) (i32.const {tag_mask})))
        (if (i32.eq (local.get $entry_tag) (i32.const {array_tag}))
          (then
            (local.set $elem_base (i32.and (local.get $elem) (i32.const {heap_mask})))
            (local.set $key
              (i32.load
                (i32.add (local.get $elem_base) (i32.const {array_header}))))
            (local.set $value
              (i32.load
                (i32.add (local.get $elem_base)
                  (i32.add (i32.const {array_header}) (i32.const {elem_shift})))))
            (local.set $key_len
              (call $value_to_string_into
                (local.get $key) (i32.const {scratch_offset})))
            (drop
              (call $property_set
