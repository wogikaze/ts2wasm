...
"#,
    ...
    pub(crate) fn emit_array_index_present(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_index(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_get_length(&self, wat: &mut String) {
  ...
  (func $property_get (param $obj i32) (param $key_ptr i32) (param $key_len i32) (result i32)
    ...
    (if (i32.eq (local.get $tag) (i32.const {array_tag}))
      (then
        ...
        (local.set $array_idx (i32.const 0))
          ...
          (else
            ...
            (block $parse_done
              (loop $parse
                ...
                (local.set $array_idx
                  (i32.add (i32.mul (local.get $array_idx) (i32.const 10)) (local.get $digit)))
          ...
          (then
            ...
            (if
              (i32.eqz
                (i32.and
                  (i32.load (i32.add (local.get $base) (i32.const {presence_words_offset})))
                  (i32.shl (i32.const 1) (local.get $array_idx))))
              (then (return (i32.const {undefined}))))
            (return
              (i32.load
                (i32.add (local.get $base)
                  (i32.add (i32.const {header}) (i32.shl (local.get $array_idx) (i32.const 2))))))))
...
"#,
    ...
    pub(crate) fn emit_property_set(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_property_delete(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_property_has(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_map_get(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_map_set(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_map_has(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_map_delete(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_add(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_has(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_delete(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_size(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_clear(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_map_size(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_prototype_add_get(&self, wat: &mut String) {
        wat.push_str(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_prototype_add_set(&self, wat: &mut String) {
        wat.push_str(
            r#"
...
"#,
    ...
    pub(crate) fn emit_set_values_array(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $set_values_array (param $set i32) (result i32)
    (local $tag i32)
    (local $set_base i32)
    (local $len i32)
    (local $i i32)
...
"#,
  ...
  (func $set_is_disjoint_from (param $set i32) (param $other i32) (result i32)
    ...
    (block $disjoint
      (loop $scan
        (br_if $disjoint (i32.ge_u (local.get $i) (local.get $len)))
        (local.set $entry_base
          (i32.add (local.get $base)
            (i32.add (i32.const {obj_header})
              (i32.shl (local.get $i) (i32.const {entry_shift})))))
        (local.set $value (i32.load (local.get $entry_base)))
        (if (i32.eq (call $set_has (local.get $other) (local.get $value)) (i32.const {true}))
          (then (return (i32.const {false}))))
        (local.set $i (i32.add (local.get $i) (i32.const {one})))
        (br $scan)))
    (i32.const {true}))
"#,
  ...
  (func $set_is_subset_of (param $set i32) (param $other i32) (result i32)
    ...
    (block $subset
      (loop $scan
        (br_if $subset (i32.ge_u (local.get $i) (local.get $len)))
        (local.set $entry_base
          (i32.add (local.get $base)
            (i32.add (i32.const {obj_header})
              (i32.shl (local.get $i) (i32.const {entry_shift})))))
        (local.set $value (i32.load (local.get $entry_base)))
        (if (i32.eq (call $set_has (local.get $other) (local.get $value)) (i32.const {false}))
          (then (return (i32.const {false}))))
        (local.set $i (i32.add (local.get $i) (i32.const {one})))
        (br $scan)))
    (i32.const {true}))
"#,
  ...
  (func $set_is_superset_of (param $set i32) (param $other i32) (result i32)
    ...
    (block $superset
      (loop $scan
        (br_if $superset (i32.ge_u (local.get $i) (local.get $len)))
        (local.set $entry_base
          (i32.add (local.get $base)
            (i32.add (i32.const {obj_header})
              (i32.shl (local.get $i) (i32.const {entry_shift})))))
        (local.set $value (i32.load (local.get $entry_base)))
        (if (i32.eq (call $set_has (local.get $set) (local.get $value)) (i32.const {false}))
          (then (return (i32.const {false}))))
        (local.set $i (i32.add (local.get $i) (i32.const {one})))
        (br $scan)))
    (i32.const {true}))
        ...
        (local.set $entry_base
          (i32.add (local.get $base)
            (i32.add (i32.const {obj_header})
              (i32.shl (local.get $i) (i32.const {entry_shift})))))
        (local.set $value (i32.load (local.get $entry_base)))
        (drop (call $set_add (local.get $result) (local.get $value)))
        (local.set $i (i32.add (local.get $i) (i32.const {one})))
        (br $scan)))
    (local.set $base (i32.and (local.get $other) (i32.const {heap_mask})))
    (local.set $len (i32.load (local.get $base)))
...
"#,
  ...
  (func $set_intersection (param $set i32) (param $other i32) (result i32)
    ...
    (local $result i32)
    (local.set $tag (i32.and (local.get $other) (i32.const {tag_mask})))
    (if (i32.ne (local.get $tag) (i32.const {object_tag})) (then (return (call $set_new))))
    (local.set $result (call $set_new))
    (local.set $base (i32.and (local.get $set) (i32.const {heap_mask})))
    (local.set $len (i32.load (local.get $base)))
    (local.set $i (i32.const {zero}))
    (block $done
      (loop $scan
        ...
        (local.set $value (i32.load (local.get $entry_base)))
        (if (i32.eq (call $set_has (local.get $other) (local.get $value)) (i32.const {true}))
          (then (drop (call $set_add (local.get $result) (local.get $value)))))
        (local.set $i (i32.add (local.get $i) (i32.const {one})))
        (br $scan)))
    (local.get $result))
"#,
...
"#,
    ...
    pub(crate) fn emit_map_values_array(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $map_values_array (param $map i32) (result i32)
    (local $tag i32)
    (local $map_base i32)
    (local $len i32)
    (local $i i32)
...
"#,
    ...
    pub(crate) fn emit_map_entry_pairs_array(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $map_entry_pairs_array (param $map i32) (result i32)
    (local $tag i32)
    (local $map_base i32)
    (local $len i32)
    (local $i i32)
    ...
    (block $done
      (loop $copy
        ...
        (i32.store
          (i32.add
            (local.get $pair_base)
            (i32.add (i32.const {array_header}) (i32.const 4)))
          (i32.load (i32.add (local.get $entry_base) (i32.const {obj_value_offset}))))
        (local.set $store_pos
          (i32.add
            (local.get $array_base)
...
"#,
    ...
    pub(crate) fn emit_weak_map_get(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_weak_map_has(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_weak_map_delete(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_weak_set_has(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
...
"#,
    ...
    pub(crate) fn emit_weak_set_delete(&self, wat: &mut String) {
