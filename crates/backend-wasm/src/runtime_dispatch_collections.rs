use super::emitter::WatEmitter;
use super::runtime_fn::RuntimeFn;

impl WatEmitter<'_> {
    /// Dispatch MapSet domain runtime functions.
    pub(super) fn emit_dispatch_collections(&mut self, f: RuntimeFn, wat: &mut String) {
        match f {
            RuntimeFn::MapNew => self.emit_map_new(wat),
            RuntimeFn::MapGet => self.emit_map_get(wat),
            RuntimeFn::MapSet => self.emit_map_set(wat),
            RuntimeFn::MapHas => self.emit_map_has(wat),
            RuntimeFn::MapDelete => self.emit_map_delete(wat),
            RuntimeFn::MapValuesArray => self.emit_map_values_array(wat),
            RuntimeFn::MapKeysArray => self.emit_map_keys_array(wat),
            RuntimeFn::MapValuesIterator => self.emit_map_values_iterator(wat),
            RuntimeFn::MapKeysIterator => self.emit_map_keys_iterator(wat),
            RuntimeFn::MapClear => self.emit_map_clear(wat),
            RuntimeFn::MapForEach => self.emit_map_for_each(wat),
            RuntimeFn::MapSize => self.emit_map_size(wat),
            RuntimeFn::MapEntriesArray => self.emit_map_entries_array(wat),
            RuntimeFn::MapEntryPairsArray => self.emit_map_entry_pairs_array(wat),
            RuntimeFn::SetNew => self.emit_set_new(wat),
            RuntimeFn::SetAdd => self.emit_set_add(wat),
            RuntimeFn::SetHas => self.emit_set_has(wat),
            RuntimeFn::SetDelete => self.emit_set_delete(wat),
            RuntimeFn::SetSize => self.emit_set_size(wat),
            RuntimeFn::SetClear => self.emit_set_clear(wat),
            RuntimeFn::SetForEach => self.emit_set_for_each(wat),
            RuntimeFn::SetIsDisjointFrom => self.emit_set_is_disjoint_from(wat),
            RuntimeFn::SetIsSubsetOf => self.emit_set_is_subset_of(wat),
            RuntimeFn::SetIsSupersetOf => self.emit_set_is_superset_of(wat),
            RuntimeFn::SetUnion => self.emit_set_union(wat),
            RuntimeFn::SetIntersection => self.emit_set_intersection(wat),
            RuntimeFn::SetDifference => self.emit_set_difference(wat),
            RuntimeFn::SetSymmetricDifference => self.emit_set_symmetric_difference(wat),
            RuntimeFn::SetFromArray => self.emit_set_from_array(wat),
            RuntimeFn::SetValuesArray => self.emit_set_values_array(wat),
            RuntimeFn::SetValuesIterator => self.emit_set_values_iterator(wat),
            RuntimeFn::SetEntriesArray => self.emit_set_entries_array(wat),
            RuntimeFn::SetPrototypeAddGet => self.emit_set_prototype_add_get(wat),
            RuntimeFn::SetPrototypeAddSet => self.emit_set_prototype_add_set(wat),
            RuntimeFn::SetPrototypeHasGet => self.emit_set_prototype_has_get(wat),
            RuntimeFn::SetPrototypeHasSet => self.emit_set_prototype_has_set(wat),
            RuntimeFn::SetPrototypeDeleteGet => self.emit_set_prototype_delete_get(wat),
            RuntimeFn::SetPrototypeDeleteSet => self.emit_set_prototype_delete_set(wat),
            RuntimeFn::SetPrototypeForEachGet => self.emit_set_prototype_for_each_get(wat),
            RuntimeFn::SetPrototypeForEachSet => self.emit_set_prototype_for_each_set(wat),
            RuntimeFn::MapPrototypeGetGet => self.emit_map_prototype_get_get(wat),
            RuntimeFn::MapPrototypeGetSet => self.emit_map_prototype_get_set(wat),
            RuntimeFn::MapPrototypeSetGet => self.emit_map_prototype_set_get(wat),
            RuntimeFn::MapPrototypeSetSet => self.emit_map_prototype_set_set(wat),
            RuntimeFn::MapPrototypeHasGet => self.emit_map_prototype_has_get(wat),
            RuntimeFn::MapPrototypeHasSet => self.emit_map_prototype_has_set(wat),
            RuntimeFn::MapPrototypeDeleteGet => self.emit_map_prototype_delete_get(wat),
            RuntimeFn::MapPrototypeDeleteSet => self.emit_map_prototype_delete_set(wat),
            RuntimeFn::MapPrototypeForEachGet => self.emit_map_prototype_for_each_get(wat),
            RuntimeFn::MapPrototypeForEachSet => self.emit_map_prototype_for_each_set(wat),
            RuntimeFn::WeakMapNew => self.emit_weak_map_new(wat),
            RuntimeFn::WeakMapSet => self.emit_weak_map_set(wat),
            RuntimeFn::WeakMapGet => self.emit_weak_map_get(wat),
            RuntimeFn::WeakMapHas => self.emit_weak_map_has(wat),
            RuntimeFn::WeakMapDelete => self.emit_weak_map_delete(wat),
            RuntimeFn::WeakSetNew => self.emit_weak_set_new(wat),
            RuntimeFn::WeakSetAdd => self.emit_weak_set_add(wat),
            RuntimeFn::WeakSetHas => self.emit_weak_set_has(wat),
            RuntimeFn::WeakSetDelete => self.emit_weak_set_delete(wat),
            RuntimeFn::WeakRefNew => self.emit_weak_ref_new(wat),
            RuntimeFn::WeakRefDeref => self.emit_weak_ref_deref(wat),
            RuntimeFn::FinalizationRegistryNew => self.emit_finalization_registry_new(wat),
            RuntimeFn::FinalizationRegistryRegister => {
                self.emit_finalization_registry_register(wat)
            }
            RuntimeFn::FinalizationRegistryUnregister => {
                self.emit_finalization_registry_unregister(wat)
            }
            _ => unreachable!("non-collections RuntimeFn routed to collections dispatch"),
        }
    }
}
