...
impl WatEmitter<'_> {
    ...
    pub(super) fn emit_dispatch_host(&mut self, f: RuntimeFn, wat: &mut String) {
        match f {
            ...
            RuntimeFn::EncodeURIComponent => self.emit_encode_uri_component(wat),
            RuntimeFn::DecodeURI => self.emit_decode_uri(wat),
            RuntimeFn::DecodeURIComponent => self.emit_decode_uri_component(wat),
    ...
    fn emit_dispatch_host_iterator(&mut self, f: RuntimeFn, wat: &mut String) {
        match f {
            ...
            RuntimeFn::IteratorMap => self.emit_iterator_map(wat),
            RuntimeFn::IteratorFilter => self.emit_iterator_filter(wat),
            RuntimeFn::IteratorTake => self.emit_iterator_take(wat),
            RuntimeFn::IteratorDrop => self.emit_iterator_drop(wat),
            RuntimeFn::IteratorToArray => self.emit_iterator_to_array(wat),
            RuntimeFn::IteratorReduce => self.emit_iterator_reduce(wat),
            RuntimeFn::IteratorForEach => self.emit_iterator_for_each(wat),
            RuntimeFn::IteratorSome => self.emit_iterator_some(wat),
