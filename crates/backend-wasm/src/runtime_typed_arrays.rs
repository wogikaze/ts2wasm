...
"#,
    ...
    /// Returns the tagged value at the given index.
    pub(super) fn emit_atomics_load(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_load (param $ta i32) (param $index_val i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32)
    ;; Check ARRAY tag
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    (local.set $index (i32.shr_s (local.get $index_val) (i32.const {number_shift})))
    ;; Bounds check
    (if (i32.ge_u (local.get $index) (i32.load (local.get $base)))
      (then (return (i32.const {undefined}))))
    (local.set $addr (i32.add
      (local.get $base)
      (i32.add (i32.const {array_header}) (i32.shl (local.get $index) (i32.const {elem_shift})))))
...
"#,
    ...
    pub(super) fn emit_atomics_store(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_store (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
...
"#,
    ...
    pub(super) fn emit_atomics_add(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_add (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32) (local $raw_old i32) (local $raw_val i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    (local.set $index (i32.shr_s (local.get $index_val) (i32.const {number_shift})))
    ...
    (local.set $raw_old (i32.shr_s (local.get $old) (i32.const {number_shift})))
    (local.set $raw_val (i32.shr_s (local.get $value) (i32.const {number_shift})))
    (i32.store (local.get $addr) (i32.shl (i32.add (local.get $raw_old) (local.get $raw_val)) (i32.const {number_shift})))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
    ...
    pub(super) fn emit_atomics_sub(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_sub (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32) (local $raw_old i32) (local $raw_val i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    ...
    (local.set $raw_old (i32.shr_s (local.get $old) (i32.const {number_shift})))
    (local.set $raw_val (i32.shr_s (local.get $value) (i32.const {number_shift})))
    (i32.store (local.get $addr) (i32.shl (i32.sub (local.get $raw_old) (local.get $raw_val)) (i32.const {number_shift})))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
    ...
    pub(super) fn emit_atomics_and(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_and (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32) (local $raw_old i32) (local $raw_val i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    ...
    (local.set $raw_old (i32.shr_s (local.get $old) (i32.const {number_shift})))
    (local.set $raw_val (i32.shr_s (local.get $value) (i32.const {number_shift})))
    (i32.store (local.get $addr) (i32.shl (i32.and (local.get $raw_old) (local.get $raw_val)) (i32.const {number_shift})))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
    ...
    pub(super) fn emit_atomics_or(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_or (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32) (local $raw_old i32) (local $raw_val i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    ...
    (local.set $raw_old (i32.shr_s (local.get $old) (i32.const {number_shift})))
    (local.set $raw_val (i32.shr_s (local.get $value) (i32.const {number_shift})))
    (i32.store (local.get $addr) (i32.shl (i32.or (local.get $raw_old) (local.get $raw_val)) (i32.const {number_shift})))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
    ...
    pub(super) fn emit_atomics_xor(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_xor (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32) (local $raw_old i32) (local $raw_val i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    ...
    (local.set $raw_old (i32.shr_s (local.get $old) (i32.const {number_shift})))
    (local.set $raw_val (i32.shr_s (local.get $value) (i32.const {number_shift})))
    (i32.store (local.get $addr) (i32.shl (i32.xor (local.get $raw_old) (local.get $raw_val)) (i32.const {number_shift})))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
    ...
    pub(super) fn emit_atomics_exchange(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_exchange (param $ta i32) (param $index_val i32) (param $value i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    (local.set $index (i32.shr_s (local.get $index_val) (i32.const {number_shift})))
...
"#,
    ...
    pub(super) fn emit_atomics_compare_exchange(&self, wat: &mut String) {
        wat.push_str(&format!(
            r#"
  (func $atomics_compare_exchange (param $ta i32) (param $index_val i32) (param $expected i32) (param $replacement i32) (result i32)
    (local $base i32) (local $index i32) (local $addr i32) (local $old i32)
    (if (i32.ne (i32.and (local.get $ta) (i32.const {tag_mask})) (i32.const {array_tag}))
      (then (return (i32.const {undefined}))))
    (local.set $base (i32.and (local.get $ta) (i32.const {heap_mask})))
    ...
    (if (i32.eq (local.get $old) (local.get $expected))
      (then (i32.store (local.get $addr) (local.get $replacement))))
    (local.get $old))
"#,
            tag_mask = ValueTag::TAG_MASK,
            array_tag = ValueTag::ARRAY,
