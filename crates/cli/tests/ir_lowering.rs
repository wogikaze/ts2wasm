...
#[test]
fn callable_interface_typed_local_reports_issue_5195() {
    let program = parse_and_resolve(
        r#"
        interface I<T> {
            <U extends T>(u: U): U;
    ...
    let result = ts2wasm_ir::lowered::lower_program(&program);
    let err = result.unwrap_err();
    assert_eq!(err.code, DiagCode::UnsupportedSyntax);
    assert!(
        err.message.contains("issue-5195"),
        "expected issue-5195 diagnostic, got: {}",
        err.message
    );
    assert!(
        err.span.is_some(),
        "expected spanned diagnostic for issue-5195"
    );
}
