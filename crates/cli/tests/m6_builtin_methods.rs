...
#[test]
fn build_smoke_number_format_integer_method() {
    let result = run_fixture("builtins-and-io/number-format-integer.ts");
    assert!(
        result.is_ok(),
        "Number formatting methods with integer arguments should compile: {:?}",
        result.err()
    );
}
