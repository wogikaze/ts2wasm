...
- `issues/open/`、`issues/done/`、またはindexジェネレータの**任意の**変更後: `mise run update-issue-index`、次に`mise run check issue-index`と`mise run check issues`
- 軽い機械的ゲートのみが必要な場合: `mise run check`（fmt + `check scripts` + `check issues`）
