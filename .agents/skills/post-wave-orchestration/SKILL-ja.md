...
- このskillで製品機能を実装しない。オーケストレーションのみ。
- 監査証拠なしでissueを作成しない。
- issue reopen後にissue-state-syncをスキップしない。

## 使用例

### 前: 監査終了時にオーケストレーションなし
...
```bash
# 残存作業を分類（親所有 vs 新規issue）
# issue indexを同期
mise run update-issue-index
# 次のwaveを決定（継続またはescalation）
# 継続の場合: 次のReady issueを選択
# escalationの場合: 人間レビュー用のblockerをドキュメント化
```
