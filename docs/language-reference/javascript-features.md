...
| `bigint` | ES2020 | heap object | literal runtime values 実装済み（decimal/binary/octal/hex, `console.log`, `typeof`, `String(bigint)`, concatenation/template ToString, truthiness）。`BigInt(...)` は signed decimal string、unsigned binary/octal/hex string、empty/whitespace-to-zero、static boolean/integer number/BigInt literal inputs、dynamic boolean/tagged-int/BigInt inputs、および dynamic definitely-string inputs の ASCII StringToBigInt subset（optional sign decimal、unsigned `0b` / `0o` / `0x`、empty/whitespace-to-zero、current single-limb/u64 range）を実装済み。literal-derived dynamic invalid/out-of-range string は issue-333 diagnostic、literal-derived nullish dynamic input は issue-280 diagnostic、broader unknown dynamic invalid/out-of-range string は issue-333 runtime abort marker を出力してから trap し、compatible exception は未実装。`BigInt.asIntN` / `BigInt.asUintN` は literal bit width `0..=64` と literal BigInt input の fold subset、同じ範囲の number/BigInt literal に束縛された単純 identifier 引数の compile-time fold、dynamic bit-width inputs `0..=64` over guarded signed-i64-backed BigInt values の runtime helper slice を実装済み。literal arithmetic は compile-time folding 対応で、`+` / `-` / `*` / `/` / `%` と非負の literal BigInt exponent `0..=64` に対する `**` を fold する。dynamic unary minus と `+` / `-` / `*` / `/` / `%` は known BigInt locals と known-local/literal operand pairs の proven-safe signed-i64-backed runtime slice のみ対応し、out-of-slice dynamic 値は issue-369 diagnostic。dynamic BigInt `**` は issue 376、BigInt `~` / `&` / `|` / `^` は issue 377、BigInt `<<` / `>>` / `>>>` は issue 378 の diagnostics を出し、ordinary number arithmetic へ lower しない。division/remainder by zero と negative BigInt exponent は compatible `RangeError` throw が issue 370 で未実装。BigInt/BigInt equality/comparison、literal BigInt/String abstract equality、literal BigInt/Boolean abstract equality、literal BigInt/Number tagged-int abstract equality、literal BigInt/nullish abstract equality、runtime BigInt/String/Boolean/nullish mixed coercion の signed-i32 StringToBigInt boundary、direct object-literal/local `valueOf: () => <bigint literal>` mixed BigInt comparison、および direct object-literal/local `toString: () => <supported decimal string>`mixed BigInt abstract equality/relational comparison は実装済み。`NaN` / `Infinity` とその signed unary forms、および `Number.NaN` / `Number.POSITIVE_INFINITY` / `Number.NEGATIVE_INFINITY` / other unshadowed `Number.*` numeric constants の statically visible BigInt/Number comparison は issue-281 diagnostic。literal-derived local/object-property out-of-range dynamic BigInt/String comparison は issue-282 diagnostic。unknown out-of-range dynamic string は issue 375、broader object `ToPrimitive` は issues 372-374 に分離済み。full multi-limb arithmetic 369 / arithmetic RangeError/TypeError parity 370 / dynamic BigInt exponentiation 376 / BigInt bitwise 377 / BigInt shifts 378 / remaining builtin edges 280 / invalid dynamic StringToBigInt exception parity 333 / mixed BigInt Number edge comparison 281 / non-BigInt primitive object coercion 372 / invalid/out-of-range object string returns 373 / broad object `ToPrimitive` 374 / unknown out-of-range dynamic BigInt/String comparison 375 は未実装 | P2 | 260, 280, 281, 282, 333, 372, 373, 374, 375, 376, 377, 378 |
...
| private fields/methods `#x` | ES2022 | private class elements | 部分実装（non-derived instance private field initializer and direct `this.#field` read/write use backend-internal private slots with Node/iwasm differential and GC-pressure coverage; direct non-derived instance private methods called as `this.#m()`, static private methods called as `this.#m()` from static methods or `Class.#m()` inside the declaring class, private getters read as `this.#x`, private setters assigned as `this.#x = value`, static private accessors read/written as `this.#x` or `Class.#x`, and direct same-class static private fields read/written as `this.#x` or `Class.#x` inside the declaring class have Node/iwasm differential coverage; static private fields and static blocks execute in source order for direct same-class `Class.#x` access, with forward static-block access diagnosed as issue-352; ordinary backing-key access/enumeration remains guarded; backend private field get/set guards the recorded private-slot count before touching private storage, but per-class brand IDs, TypeError throwing, derived/extracted/external full brand checks, and accessor duplicate-pair semantics remain issue-351/issue-255） | P2 | 248, 255, 351, 352 |
