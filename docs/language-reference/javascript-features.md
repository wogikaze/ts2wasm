...
BigInt mixed comparison の object `ToPrimitive` boundary: direct object-literal/local no-argument arrow `valueOf` / `toString` returning supported primitive literals is implemented. The accepted next narrow slice is own no-argument method syntax whose body is a single primitive `return`, preserving ordinary `valueOf` before `toString` ordering. Prototype lookup, inherited methods, getters, Proxy traps, receiver-sensitive method bodies, side effects, and exception-producing coercion remain unsupported until the broader object model and completion propagation contracts are implemented. These follow-ups are tracked by issues 374 and 5130.
