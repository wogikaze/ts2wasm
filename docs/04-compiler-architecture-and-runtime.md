...
BigInt は heap object representation を採用する。`RawValue` の low-bit tag は増やさず、object-tagged heap pointer の header kind で BigInt を判別する。literal allocation、arithmetic、comparison/coercion、builtin/string conversion は `docs/14-runtime-abi.md` の BigInt ABI boundary と issue 256-259 の実装 slice で段階的に導入する。
