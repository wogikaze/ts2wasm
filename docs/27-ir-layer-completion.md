...
2026-05-13 時点で、この領域は「設計だけ」ではない。すでに operational な部分と、bridge のまま残っている部分が混在している。
...
現状の native `MirProgram` は `LoweredProgram` と別型だが、backend parity のため
`LoweredProgram` との変換 bridge を保持している。この bridge は有効だが、完成形ではない。
