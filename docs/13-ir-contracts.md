...
MIR (Mid-level IR / Runtime IR) — native data model / validator 実装済み、emitter は bridge
  `MirProgram` / `MirFunction` / `MirStmt` / `MirExpr` は `LoweredProgram` とは別型である。
  `Validated<MirProgram>` と optional `emit_mir_wat` path は存在するが、native MIR emitter は次段の作業である。
...
## MIR — Mid-level IR（native data model / validator 実装済み、emitter は bridge）
