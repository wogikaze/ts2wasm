...
| `bigint_mul` / `div` / `rem` | `(jsval, jsval) -> jsval` | issue 260/263/384; future issue 369/370 | `mul` remains the signed-i64-safe first-limb reconstruction slice. `div` / `rem` now use cached decimal long division for known BigInt operands outside signed i64 and preserve truncation, quotient sign, remainder sign, and canonical zero. Control-flow-assigned unproved BigInt locals remain issue 398; division/remainder by zero currently trap and RangeError parity is issue 370 |
