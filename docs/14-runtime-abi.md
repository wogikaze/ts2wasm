...
BigInt object payload は canonical little-endian limb sequence とする。Issue 259 implements the literal runtime slice for values that fit the current first-limb backend constructor path and stores a cached decimal representation after the canonical prefix for observable literal printing and `String(<literal>)`. Issue 382 adds the first dynamic `+`/`-` slice outside signed-i64 by operating on the cached decimal payload for known BigInt operands, including branch-assigned result locals; unary-minus and `*`, `/`, `%` still reconstruct through signed i64 only when pre-lowering proves the operands/results fit that helper slice. Full canonical multi-limb runtime arithmetic for the remaining operators is tracked by issue 369 before broad compatibility can be claimed.
...
| `bigint_add` / `sub` | `(jsval, jsval) -> jsval` | issue 260/382; future issue 369/370 | Known BigInt operands now support signed-i64-safe values plus the first cached-decimal multi-limb add/sub slice outside signed-i64, including branch-assigned result locals. Number 混在 is split to issue 370 for TypeError parity; broader canonical limb fallback remains issue 369 |
...
| `bigint_bitwise_not` / `and` / `or` / `xor` | `(jsval[, jsval]) -> jsval` | issue 387; future issue 369/370 | Progress slice implemented for known BigInt operands/results proven signed-i64-safe through first-limb reconstruction. Static BigInt literal `~` folds with arbitrary decimal magnitude using `~x == -x - 1`, so results outside the signed-i64 helper boundary can still lower as canonical BigInt literals. Mixed Number/BigInt, unproved dynamic values, binary bitwise results outside the current signed-i64 constructor boundary stay diagnosed; operators never lower through ordinary number bitwise helpers. |
...
- Lowering: literal は `BigIntLiteral { raw, radix, negative }` 相当の semantic/lowered node へ落とし、backend が runtime constructor を選ぶ。Known BigInt `+` / `-` lower to runtime helpers for signed-i64-safe values and the current cached-decimal multi-limb add/sub slice; unary minus, `*`, `/`, `%`, signed-i64-safe `**`, and signed-i64-safe BigInt-specific `~` / `&` / `|` / `^` still require pre-lowering proof that they remain inside the signed-i64 helper slice. Literal BigInt `**` is folded before lowering only for non-negative exponent literals in `0..=64`; dynamic BigInt exponentiation outside the signed-i64-safe proof boundary remains issue 376. BigInt bitwise helpers do not share ordinary number bitwise lowering. BigInt operation mixed Number/BigInt TypeError parity is split to issue 370; current statically visible mixes remain issue-linked diagnostics
