...
| D | W5 | test262 の executed count が 100 件以上。`artifacts/coverage/reference-coverage-matrix.md` が最新。`scripts/gen/coverage-matrix.sh --check` が通る |
...
| G | W7 | benchmark suite が固定されている。`scripts/perf/benchmark-tracker.sh` が median / p95 / wasm_size を記録する。前回比で regression が出た場合に gate が落ちる |
