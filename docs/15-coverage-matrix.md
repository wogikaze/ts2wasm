...
- PR では coverage gate (`scripts/gen/coverage-matrix.sh --check`) を実行し、matrix 未更新を失敗扱いにする。
...
- 実際に `scripts/run/reference-coverage.sh` で走らせた件数。
...
- `artifacts/coverage/reference-coverage-matrix.md` must match `scripts/gen/coverage-matrix.sh --check` output
