...
BigInt は heap object representation として設計済みだが、runtime 値と操作は段階実装である。BigInt literal runtime values は issue 259、arithmetic は issue 260、equality/comparison/coercion は issue 261、builtin/string conversion は issue 262 が所有する。BigInt と Number の arithmetic は暗黙変換せず TypeError path にする。現在の issue-261 progress slice は BigInt 同士の mathematical value strict equality、abstract equality、relational comparison と、Number など非 BigInt との `===` / `!==` false/true 境界を実装済みである。Literal BigInt/String `==` / `!=` は supported StringToBigInt subset で fold し、invalid string は Node と同じ false/true 境界にする。その他の mixed BigInt abstract equality と relational comparison は ECMA-262 の BigInt/Number/String coercion rules が必要なため、dynamic StringToBigInt parsing、broader number model limits (`NaN`, `Infinity`, `-0`, fractional)、object `ToPrimitive` が揃うまで statically visible case は issue-261 diagnostic、runtime-only case は trap として止める。
