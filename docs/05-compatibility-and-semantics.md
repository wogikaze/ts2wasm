...
Object literal/local mixed BigInt comparisons that require `ToPrimitive` through `valueOf` / `toString` report source-spanned diagnostics; compatible object coercion and non-source-backed unknown out-of-range runtime strings are split to issue 368.
