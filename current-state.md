...
- ABC451 D is not sample-compatible yet. The rewritten fixture builds past the previous `NumberOutOfRange` blocker and the array-push/free-list reducers pass. Issue 303 raised the bounded standalone memory cap from 16 to 42 pages: the depth-7 ABC451 live-set reducer now prints Node-matching `61002` under `iwasm`, while the OOM regression still traps intentionally. The full official sample inputs `10`, `69`, and `1099898` still reach `$alloc_heap` `Exception: unreachable`, so issue 300 remains open for the next ABC451 runtime blocker.
