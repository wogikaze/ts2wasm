...
- JSON has a documented supported-subset contract in issue 052. `JSON.parse` covers whitespace-trimmed primitives, supported strings, supported numbers, arrays/objects with nested supported values, ASCII escapes, ASCII-valued `\uXXXX` escapes, and many malformed-input rejection paths. `JSON.stringify` covers representable primitives, arrays/objects including nested literals, supported string escaping, numeric/string/ignored/selected boxed `space` forms, and object-literal array replacers with string/numeric property-list entries. Full JSON compatibility remains blocked on child issues 052b through 052f for arbitrary non-integer numbers, UTF-16/surrogates, broader replacer semantics, boxed/coercion edge cases, and throw-compatible parse diagnostics.
