...
- ABC451 D is not sample-compatible yet. The rewritten fixture builds past the previous `NumberOutOfRange` blocker and the array-push/free-list reducers pass, but the full official sample path still reaches `$alloc_heap` under the committed 16-page memory cap. A depth-7 ABC451 live-set reducer prints `61002` with only a temporary `MEMORY_MAX_PAGES=512` change and traps under the committed cap; issue 303 owns the runtime memory-limit policy decision and must preserve an explicit OOM boundary.
