...
- GC 実装は未完。`$alloc_heap` は GC header と allocation threshold hook を持ち、GC mark phase は module cache / class prototype globals / heap graph payload を走査する。sweep/free-list reuse は issue 219 の追跡対象。OOM check と UTF-8 literal basic support は完了済みだが、UTF-16 parity / encode-decode helper は追跡対象。
