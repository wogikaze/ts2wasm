...
- BigInt literal syntax is recognized by the frontend for decimal, binary, octal, and hexadecimal forms with `n` suffix. Runtime BigInt values remain unsupported and issue-linked rather than being mis-tokenized as a number plus identifier.
...
- BigInt literals are parser-classified as explicit AST nodes and invalid non-separator forms such as fractional/exponent BigInt, invalid radix digits, and decimal leading-zero BigInt produce issue-244 diagnostics. BigInt runtime representation, arithmetic, equality, and builtin behavior remain tracked by issue 250.
