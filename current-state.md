...
- ABC451 D is not sample-compatible yet. The rewritten fixture builds past the previous `NumberOutOfRange` blocker and the array-push/free-list reducers pass. Issue 304 raised the bounded standalone memory cap from 42 to 185 pages: the depth-8 ABC451 live-set reducer now prints Node-matching `292743` under `iwasm`, while the OOM regression still traps intentionally. The full official sample inputs `10`, `69`, and `1099898` still require issue 300 follow-up before compatibility can be claimed.
