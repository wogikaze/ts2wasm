...
- BigInt literal syntax is recognized by the frontend for decimal, binary, octal, and hexadecimal forms with `n` suffix. BigInt runtime representation is designed as a heap object in `docs/14-runtime-abi.md`; runtime literal values, arithmetic, equality/comparison, and builtin/string conversion remain unsupported and tracked by issues 259-262 rather than being mis-tokenized as a number plus identifier.
...
- BigInt literals are parser-classified as explicit AST nodes and invalid non-separator forms such as fractional/exponent BigInt, invalid radix digits, and decimal leading-zero BigInt produce issue-244 diagnostics. BigInt runtime representation is designed as a heap object with object-tagged `RawValue` and canonical sign/limb payload; implementation remains split across issues 259 (literal values), 260 (arithmetic), 261 (equality/comparison/coercion), and 262 (builtin/string conversion).
