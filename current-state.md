...
- issue 060 progress classified current `unknown-unsupported` coverage windows into concrete feature labels; stored matrix rows now include test262 limit 4000, tsc limit 200, and tsgo limit 120. The test262 limit-4000 row has zero `unknown-unsupported`; no new classifier labels were needed beyond the limit-1250 `async-iteration` slice. The exact assigned tsc root `/home/wogikaze/wgkz/ts2wasm/reference` lacks `TypeScript`, so the stored tsc row was refreshed with the existing `/tmp/ts2wasm-issue060-reference` checkout.
