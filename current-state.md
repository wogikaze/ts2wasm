...
- BigInt literal syntax is recognized by the frontend for decimal, binary, octal, and hexadecimal forms with `n` suffix. BigInt runtime representation is designed as a heap object in `docs/14-runtime-abi.md`; runtime literal values, arithmetic, equality/comparison, and builtin/string conversion remain unsupported and tracked by issues 256-259 rather than being mis-tokenized as a number plus identifier.
