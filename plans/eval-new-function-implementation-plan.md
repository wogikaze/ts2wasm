...
| `caller_is_strict` が未接続 | builtin resolution now annotates `EvalFragmentPlan.caller_is_strict` across script/function/class/arrow/nested expression scopes, and compiler eval-expand uses strict caller context for static eval expansion and strict-caller `var` locality | dynamic direct eval host descriptor で strict metadata を使い切る |
...
| `new eval` | `new eval("x")` | `eval is not constructor` TypeError parity is guarded through a catchable node-shim fixture | broader non-constructor builtin parity audit |
| literal `Function` | `Function("a", "return a")` | resolver now emits an explicit `ResolvedExpr::FunctionConstructor` node before compiler expansion; nested function/class body traversal, zero-arg, caller-local non-capture, non-simple duplicate bound-name and strict-body duplicate/non-simple/eval/arguments parameter early errors, default/rest/destructuring parameter execution with Node `.length` metadata, `arguments` object reads, constructor object-return behavior, strict/sloppy `this` binding, construct-call `new.target` typeof / `.name` / `.length` behavior, and direct `.name` / `.length` / `.prototype` metadata are guarded for static constructor locals | first-class static `FunctionConstructorPlan` / generated function object |
| literal `new Function` | `new Function("a", "return a")` | resolver now emits an explicit `ResolvedExpr::FunctionConstructor` node before compiler expansion; zero-arg and call output are Node differential guarded | generated function object + metadata |
...
- strict caller / strict eval code を伝搬する。Builtin resolution now annotates
  `EvalFragmentPlan.caller_is_strict` across script/function/class/arrow scopes,
  and static eval expansion keeps strict-caller `var` declarations eval-local;
  remaining work is to consume this metadata in the dynamic direct-eval host
  descriptor.
...
- `ResolvedExpr::FunctionConstructor` を追加する。Resolved IR node is now present
  for both static and dynamic intrinsic `Function` / `new Function` shapes; the
  remaining `FunctionConstructorPlan` work is a richer parse/metadata plan rather
  than magic identifier call/new plumbing.
...
- `host.eval.direct` manifest / host-deny checks are implemented, and a
  ...
  Dynamic direct eval inside arrow functions now captures the enclosing lexical
  env-cell descriptor, including lexical `this`, `arguments`, runtime source
  locals, self bindings, and outer lexical locals needed for write-back through
  the focused Node shim. Host-deny regressions cover the arrow lexical read and
  write-back host lane.
  Focused dynamic direct eval regressions now cover `var` and function
  declarations written back through an existing caller `var` landing zone and
  re-read by a subsequent dynamic direct eval. The Node shim preserves host
  function handles when decoding env descriptor values so declaration-created
