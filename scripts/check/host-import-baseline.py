...
VARIANT_TO_CAPABILITY = {
    "FdRead": "wasi.fd", "FdWrite": "wasi.fd", "PathOpen": "wasi.filesystem",
    "FdClose": "wasi.fd", "FdSeek": "wasi.fd", "FdPrestatGet": "wasi.fd",
    "FdPrestatDirName": "wasi.fd", "WasiProcExit": "wasi.proc_exit",
    "ClockTimeGet": "wasi.clock", "ClockResGet": "wasi.clock",
    "RandomGet": "wasi.random", "ArgsSizesGet": "wasi.args", "ArgsGet": "wasi.args",
    "EnvironSizesGet": "wasi.env", "EnvironGet": "wasi.env",
    "PathCreateDirectory": "wasi.filesystem", "PathFilestatGet": "wasi.filesystem",
    "PathReadlink": "wasi.filesystem", "PathRemoveDirectory": "wasi.filesystem",
    "PathRename": "wasi.filesystem", "PathSymlink": "wasi.filesystem",
    "PathUnlinkFile": "wasi.filesystem",
    "FsReadFileSync": "host.fs.readFileSync", "FsWriteFileSync": "host.fs.writeFileSync",
    "FsAppendFileSync": "host.fs.appendFileSync", "ProcessExit": "host.process.exit",
    "PathJoin": "host.path.join", "PathResolve": "host.path.resolve",
    "PathBasename": "host.path.basename", "PathDirname": "host.path.dirname",
    "CryptoRandomBytes": "host.crypto.randomBytes", "EncodeURI": "host.encodeURI",
    "DecodeURI": "host.decodeURI", "Escape": "host.escape", "Unescape": "host.unescape",
    "DateToString": "host.dateToString", "DateGetLocalTimeField": "host.dateGetLocalTimeField",
...
def collect_catalog_imports():
    ...
    for m in spec_pattern.finditer(content):
        cap = VARIANT_TO_CAPABILITY.get(m.group(1), f"unknown.{m.group(1)}")
        imports.append({
            "module": m.group(2), "name": m.group(3), "wat_symbol": m.group(4),
            "abi": m.group(5), "capability": cap, "reason_owner": "runtime-catalog",
        })
...
def diff_imports(catalog_imports, baseline_imports):
    ...
    changed = []
    for key in sorted(catalog_keys & baseline_keys):
...
def main():
    ...
    if added or removed or changed:
        ...
        print(f"  Catalog: {len(catalog_imports)}, Baseline: {len(baseline_imports)}")
        ...
        for key in changed:
            print(f"    ~ {key[0]}.{key[1]}")
        print("Run with --update to refresh baseline.")
        sys.exit(1)
