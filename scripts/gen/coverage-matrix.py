...
def render_row(result: dict, suite_key: str) -> str:
    ...
    evidence = result.get("evidence", f"scripts/run/reference-coverage.sh {suite_key} --limit {executed}")

    return f"| {suite_name} | {denominator} | {executed} | {build_cov} | {semantic_cov} | {build_pass} | {semantic_pass} | {fail} | {unsupported} | {blocked} | {skip} | {diagcodes} | in-progress | `{evidence}` |"
...
def render_empty_row(suite_key: str, config: dict) -> str:
    ...
    step = config["step"]
    return f"| {suite_name} | 0 | 0 | 0.00 | 0.00 | 0 | 0 | 0 | 0 | 0 | 0 | - | in-progress | `scripts/run/reference-coverage.sh {suite_key} --limit {step}` |"
...
def generate_matrix() -> str:
    ...
    header = "| suite | denominator | executed | build_coverage% | semantic_coverage% | build_pass | semantic_pass | fail | unsupported | blocked | skip-with-reason | unsupported (DiagCode breakdown) | status | evidence |"
    separator = "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---|"
...
Generated by scripts/gen/coverage-matrix.py.
