---
id: 2978
title: "Implement Intersectionsoflargeunions Import Export"
type: spike
...
Triage intersectionsOfLargeUnions-import-export across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `intersectionsOfLargeUnions-import-export` with diagnostics: import-export. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: intersectionsOfLargeUnions-import-export has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/intersectionsOfLargeUnions.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/intersectionsOfLargeUnions.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/intersectionsOfLargeUnions.ts`
