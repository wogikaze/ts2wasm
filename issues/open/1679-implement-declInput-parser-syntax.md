---
id: 1679
title: "Implement Declinput Parser Syntax"
type: spike
...
Triage declInput-parser-syntax across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `declInput-parser-syntax` with diagnostics: parser-syntax. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: declInput-parser-syntax has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/declInput.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/declInput.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/declInput.ts`
- `reference/typescript/tests/cases/compiler/declInput3.ts`
