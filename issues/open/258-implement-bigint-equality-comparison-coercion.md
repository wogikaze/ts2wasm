...
- [ ] Implement strict equality for BigInt mathematical values.
...
- [ ] Implement relational comparison for BigInt/BigInt and supported primitive mixed cases.
...
- [ ] Node/iwasm differential fixtures cover `===`, `!==`, `==`, `!=`, `<`, `<=`, `>`, and `>=` for BigInt operands.
...
- [ ] Docs/current-state/issues state the remaining object `ToPrimitive` and number-model limits.
