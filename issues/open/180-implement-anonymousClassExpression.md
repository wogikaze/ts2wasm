...
> Evidence: `git log --oneline --all --grep=180` shows only creation/chore commits — no feat/fix commit.
...
Failing test: `anonymousClassExpression1.ts` — anonymous class expression
...
Test  now build_pass. Fix implemented by Agent 1 (parallel session): anonymous class expression support added to the parser.
