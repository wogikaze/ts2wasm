...
- [ ] Fix the invalid format string or missing placeholder in the array-push growth WAT emission path.
- [ ] Preserve existing Array.prototype.push growth behavior.
- [ ] Add or update a narrow regression test only if the existing array-push coverage does not exercise this path.
...
- `issues/open/356-fix-array-push-growth-wat-format-compile-blocker.md`
- `issues/index.md`
...
- [ ] `cargo check -p ts2wasm-backend-wasm` passes.
- [ ] `cargo nextest run -E 'test(bigint) or test(node_diff)'` no longer fails on this `expr_emit.rs` compile blocker.
- [ ] Existing array-push coverage remains valid; add a regression only if no existing test covers the fixed path.
- [ ] Issue 280 remains open unless its own acceptance criteria are separately verified.
