...
`issues/open/3500-implement-newLexicalEnvironmentForConvertedLoop.md`.
...
For-of array binding pattern heads are parsed correctly.

Commits:
- Parser detects LeftBracket in for-in/of look-ahead for array binding patterns

Validation:
```sh
echo 'for (const [key, value] of Object.entries({a:1})) {}' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
