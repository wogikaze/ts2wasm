...
Class constructor parameters are preserved for new arity validation.

Commits:
- Constructor parameters are tracked for arity checking

Validation:
```sh
echo 'class C { constructor(x: number) { this.x = x; } }; new C(1);' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
