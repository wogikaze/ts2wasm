...
Reference test results show 1 cases fail in directory `nestedFreshLiteral` with diagnostics: parser-syntax. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: nestedFreshLiteral has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/nestedFreshLiteral.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/nestedFreshLiteral.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/nestedFreshLiteral.ts`
