---
id: 1287
title: "Implement Collisionexportsrequireandambientfunction"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1287.

## Summary

Closed as stale. Fresh triage and focused coverage show
`reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts`
...
`collisionExportsRequireAndAmbientFunction` with diagnostics: import-export.
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts --detail --no-dashboard-data
```
...
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts --detail
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts
```
...
### Smart triage: Build pass: collisionExportsRequireAndAmbientFunction

- Issue class: `none`
- Feature label: `build-pass`
- Diagnostic: `BuildPass` / `pass`
- Path: `reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts`
...
```text
tokens: ok; includes export declare function exports/require, declare namespace m1, namespace m2, and binding a
...
visible symbols: function exports/require in module and namespace scopes plus binding a initialized to 10
```
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts
...
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientFunction.ts --detail --no-dashboard-data
