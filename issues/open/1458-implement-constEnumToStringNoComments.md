---
id: 1458
title: "Implement Constenumtostringnocomments"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1458.
...
`toString` bucket. Current compiler behavior does not reach enum member
constant values, property/string-index access, `.toString()` calls, comment
removal, or const-enum inlining because parsing stops at the first
`const enum Foo` declaration.

Problem: `constEnumToStringNoComments.ts` is blocked by const-enum declaration
parsing before toString/no-comments behavior can be triaged.
...
```sh
env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/constEnumToStringNoComments.ts
```

Coverage window:

```sh
env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/constEnumToStringNoComments.ts --detail --no-dashboard-data
```
...
- Comment stripping or emit behavior.
...
### Smart triage: Triage enum: constEnumToStringNoComments

- Issue class: `triage-needed`
- Feature label: `enum`
- Diagnostic: `UnsupportedSyntax` / `parser-or-frontend-unsupported`
- Path: `reference/typescript/tests/cases/compiler/constEnumToStringNoComments.ts`
...
```text
UnsupportedSyntax: const declarations require an initializer at 52..56
```
...
```text
command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/constEnumToStringNoComments.ts
result: pass; current first blocker is the same const-enum parser support tracked by issue 5184
date: 2026-05-07

command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/constEnumToStringNoComments.ts --detail --no-dashboard-data
...
- After issue 5184 advances const-enum parsing, this file may expose enum member
  ...
  `.toString()` behavior, or comment-removal emit work as later blockers.
