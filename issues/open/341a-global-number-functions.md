...
- [ ] `isNaN(x)` — return true if argument coerces to NaN, false otherwise
- [ ] `parseInt(s, radix?)` — parse string-integer with optional radix (2-36)
- [ ] `parseFloat(s)` — parse string-decimal
- [ ] `isFinite(x)` — return false if argument coerces to NaN/+Inf/-Inf, true otherwise
- [ ] Node/iwasm differential fixtures for each function
- [ ] Each function recognizable in name resolver and lowered to a RuntimeCall
...
- [ ] Node/iwasm differential fixture `fixtures/builtins-and-io/global-isnan.ts` matches Node output for `isNaN(NaN)`, `isNaN(42)`, `isNaN("x")`, `isNaN(undefined)`
- [ ] Node/iwasm differential fixture `fixtures/builtins-and-io/global-parseint.ts` matches Node output for `parseInt("42")`, `parseInt("0xFF", 16)`, `parseInt("  101", 2)`, `parseInt("abc")`
- [ ] Node/iwasm differential fixture `fixtures/builtins-and-io/global-parsefloat.ts` matches Node output for `parseFloat("3.14")`, `parseFloat("   -0.5")`, `parseFloat("abc")`
- [ ] Node/iwasm differential fixture `fixtures/builtins-and-io/global-isfinite.ts` matches Node output for `isFinite(42)`, `isFinite(Infinity)`, `isFinite(NaN)`, `isFinite("42")`
- [ ] `cargo fmt --all --check` and targeted `cargo nextest run -E 'test(builtin) or test(node_diff)'` pass
