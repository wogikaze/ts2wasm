---
id: 3932
title: "Implement Reachabilitychecks Name Resolution"
type: spike
...
Triage reachabilityChecks-name-resolution across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `reachabilityChecks-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: reachabilityChecks-name-resolution has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/reachabilityChecks4.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/reachabilityChecks4.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/reachabilityChecks4.ts`
- `reference/typescript/tests/cases/compiler/reachabilityChecks8.ts`
