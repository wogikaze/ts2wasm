...
> Reason: Created directly in `issues/done/` with zero implementation work. Batch audit `3f0bfdf18` stamped it as truly-done without verification. No ABI golden test infrastructure exists.
> Evidence: `git log --oneline --all --grep=5051` shows only the creation commit — no feat/fix commit.
> Required to close: ABI layout golden tests and versioning in `crates/runtime-abi/src/`.
