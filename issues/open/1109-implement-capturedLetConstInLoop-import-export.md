...
> Evidence: Empty completion evidence. No feat/fix commit for #1109.

## Summary

Triage capturedLetConstInLoop-import-export across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `capturedLetConstInLoop-import-export` with diagnostics: import-export. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: capturedLetConstInLoop-import-export has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/capturedLetConstInLoop4.ts --detail
```
