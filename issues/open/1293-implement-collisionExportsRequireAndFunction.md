---
id: 1293
title: "Implement Collisionexportsrequireandfunction"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1293.

## Summary

Closed as stale. Fresh triage and focused coverage show
`reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts`
...
`collisionExportsRequireAndFunction` with diagnostics: import-export. Fresh
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts --detail --no-dashboard-data
```
...
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts --detail
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts
```
...
### Smart triage: Build pass: collisionExportsRequireAndFunction

- Issue class: `none`
- Feature label: `build-pass`
- Diagnostic: `BuildPass` / `pass`
- Path: `reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts`
...
```text
tokens: ok; includes export function exports/require and namespace m1/m2 function declarations
ast: ok; ExportDecl Function exports and ExportDecl Function require
resolved: ok; Function exports returns Number(1), Function require returns String("require")
visible symbols: function exports/require in module and namespace scopes
```
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts
...
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndFunction.ts --detail --no-dashboard-data
