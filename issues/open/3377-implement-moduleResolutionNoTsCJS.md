---
id: 3377
title: "Implement Moduleresolutionnotscjs (audit reopened #3377)"
type: maintenance
...
Triage moduleResolutionNoTsCJS across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 case in `moduleResolutionNoTsCJS`. Fresh triage
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts`
...
env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm \
  ...
  --path-filter reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts \
...
per-file: moduleResolutionNoTsCJS.ts => UnsupportedModule / import-export
```
...
message: tried .../reference/typescript/tests/cases/compiler/./x.ts at 272..280
source:
// @filename: x.ts
export default 0;
...
```text
command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts --detail --no-dashboard-data
result: pass; reproduced UnsupportedModule/import-export first blocker
date: 2026-05-08

command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/moduleResolutionNoTsCJS.ts
...
- After issue `5229`, this reference may advance to the intended TS2307
  ...
  CommonJS, or to duplicate default export diagnostics.

## Close note

Superseded by issue `5229`. Fresh triage reaches module graph construction and
fails to resolve `./x.ts` to the sibling virtual `x.ts` section before
CommonJS no-TS-extension diagnostics are actionable.
...
Evidence files:
- `issues/open/3377-implement-moduleResolutionNoTsCJS.md` before this move
- `issues/open/3377-implement-moduleResolutionNoTsCJS.md` after this move
