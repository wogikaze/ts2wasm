...
- This generated parent is already split by narrower FunctionDeclaration issues and must not be selected directly.
- Parser-syntax cases `FunctionDeclaration3.ts`, `FunctionDeclaration4.ts`, and `FunctionDeclaration6.ts` are covered by open issue `550-implement-FunctionDeclaration-parser-syntax.md`.
- Import/export case `FunctionDeclaration7.ts` is covered by done issue `549-implement-FunctionDeclaration-import-export.md`.
- The representative failure in this parent is the overload/ambient declaration shape `function foo();`, which issue 550 already carries with exact triage evidence.
...
- issue-state commit closing this generated bucket as superseded by issues 550 and 549
...
- Parser implementation for `function foo();` style declarations remains open in issue 550.
