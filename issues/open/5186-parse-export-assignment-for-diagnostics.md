...
- `issues/open/3335-implement-moduleElementsInWrongContext.md` has two
...
Export assignment expressions are parsed and produce Stmt::ExportAssignment. Module resolution not implemented but parsing is correct.

Commits:
- Parser handles `export = expr;` syntax

Validation:
```sh
echo 'export = 42;' | ts2wasm build --stdin -o /tmp/out.wasm
# => error: [UnsupportedModule] issue-055 (expected - module dispatch)
```
