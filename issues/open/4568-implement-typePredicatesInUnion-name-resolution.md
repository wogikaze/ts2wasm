---
id: 4568
title: "Implement Typepredicatesinunion Name Resolution"
type: spike
...
Triage typePredicatesInUnion-name-resolution across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `typePredicatesInUnion-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: typePredicatesInUnion-name-resolution has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/typePredicatesInUnion2.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/typePredicatesInUnion2.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/typePredicatesInUnion2.ts`
- `reference/typescript/tests/cases/compiler/typePredicatesInUnion3.ts`
