...
Reference test results show 1 case failing in directory `allowJscheckJsTypeParameterNoCrash` with diagnostics: import-export. Fresh triage shows the current failure is the initialized `export var vextend = extend;` issue-055 boundary already owned by issue 5285.

Problem: `allowJscheckJsTypeParameterNoCrash` had 1 generated bucket failure and needed smart-triage evidence. No new child is needed because issue 5285 already owns the current blocker.
...
- [x] Existing issue 5285 contains the implementation-ready initialized `export var name = expr;` owner
...
[
  ...
  {
    "state": "open",
    "path": "issues/open/513-implement-allowJscheckJsTypeParameterNoCrash.md",
    "title": "Implement Allowjscheckjstypeparameternocrash",
...
```text
command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/allowJscheckJsTypeParameterNoCrash.ts --detail --no-dashboard-data
...
command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/allowJscheckJsTypeParameterNoCrash.ts
result: pass; tokens succeed, current blocker is issue-055 initialized export var, superseded by issue 5285
date: 2026-05-08
```
...
```text
UnsupportedModule: issue-055: unsupported variable export; module resolution and loading are not implemented at 329..335
```
