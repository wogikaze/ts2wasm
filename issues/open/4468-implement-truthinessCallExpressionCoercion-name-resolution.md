---
id: 4468
title: "Implement Truthinesscallexpressioncoercion Name Resolution"
type: spike
...
Triage truthinessCallExpressionCoercion-name-resolution across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `truthinessCallExpressionCoercion-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: truthinessCallExpressionCoercion-name-resolution has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/truthinessCallExpressionCoercion4.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/truthinessCallExpressionCoercion4.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/truthinessCallExpressionCoercion4.ts`
