...
Triage string-builtin feature across 611 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 611 cases fail with string-builtin diagnostic. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: string-builtin feature has 611 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/String/prototype/big/length.js
```

Coverage window:

```sh
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/String/prototype/big/length.js --detail
```
...
```sh
mise run reference-coverage -- test262 --limit 1222
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/String/prototype/big/length.js --detail
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/String/prototype/big/length.js
```
...
- `issues/open/448-implement-string-builtin.md` - Implement string-builtin support (same reference path, same feature label, same group key, title overlap)
...
`issues/open/5021-implement-string-builtin.md` に統合されました。
そちらを参照してください。
## Completion evidence
...
Evidence files:
- `issues/open/5088-implement-string-builtin.md`
