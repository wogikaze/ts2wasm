...
- Implementation commits: verified via `git log --oneline --all --grep=5357`
...
Qualified Function constructors avoid the eval diagnostic.

Commits:
- Name resolver only reports eval diagnostic for unqualified `Function` calls

Validation:
```sh
echo 'let F: any = Function; new F("");' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
