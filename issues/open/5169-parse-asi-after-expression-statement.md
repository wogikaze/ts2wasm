...
Also supersedes generated bucket `issues/open/1183-implement-classExpressionNames.md`: fresh triage for `classExpressionNames.ts` stops after the completed expression statement `A = class {}` before the later-line `var a = new A()`, which is the same ASI-after-expression-statement parser boundary.
