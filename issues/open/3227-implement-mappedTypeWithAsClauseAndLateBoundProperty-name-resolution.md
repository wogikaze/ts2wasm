---
id: 3227
title: "Implement Mappedtypewithasclauseandlateboundproperty Name Resolution"
type: spike
...
Triage mappedTypeWithAsClauseAndLateBoundProperty-name-resolution across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `mappedTypeWithAsClauseAndLateBoundProperty-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: mappedTypeWithAsClauseAndLateBoundProperty-name-resolution has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/mappedTypeWithAsClauseAndLateBoundProperty.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/mappedTypeWithAsClauseAndLateBoundProperty.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/mappedTypeWithAsClauseAndLateBoundProperty.ts`
