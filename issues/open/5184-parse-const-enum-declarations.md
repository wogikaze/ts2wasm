...
> Evidence: Empty completion evidence. No feat/fix commit for #5184.
...
- `issues/open/1448-implement-constEnumExternalModule.md` reaches the same
...
Stmt::EnumDecl implemented for const enum declarations. Parser produces Stmt::EnumDecl, name resolver registers enum names.
...
Validation:
```sh
echo 'const enum E { A, B }; let x: any = E' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0 (enum name resolves)
```
