---
id: 2406
title: "Implement Fixingtypeparametersrepeatedly Name Resolution"
type: spike
...
Triage fixingTypeParametersRepeatedly-name-resolution across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `fixingTypeParametersRepeatedly-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: fixingTypeParametersRepeatedly-name-resolution has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/fixingTypeParametersRepeatedly3.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/fixingTypeParametersRepeatedly3.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/fixingTypeParametersRepeatedly3.ts`
- `reference/typescript/tests/cases/compiler/fixingTypeParametersRepeatedly1.ts`
