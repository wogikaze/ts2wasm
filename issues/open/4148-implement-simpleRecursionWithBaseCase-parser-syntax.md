---
id: 4148
title: "Implement Simplerecursionwithbasecase Parser Syntax"
type: spike
...
Triage simpleRecursionWithBaseCase-parser-syntax across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `simpleRecursionWithBaseCase-parser-syntax` with diagnostics: parser-syntax. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: simpleRecursionWithBaseCase-parser-syntax has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/simpleRecursionWithBaseCase3.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/simpleRecursionWithBaseCase3.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/simpleRecursionWithBaseCase3.ts`
- `reference/typescript/tests/cases/compiler/simpleRecursionWithBaseCase4.ts`
