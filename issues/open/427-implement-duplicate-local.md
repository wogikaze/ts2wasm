...
Triage duplicate-local feature across 252 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 252 cases fail with duplicate-local diagnostic. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: duplicate-local feature has 252 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-coverage -- test262 --limit 504
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/Object/is/emulates-undefined.js --detail
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/Object/is/emulates-undefined.js
```
...
### Smart triage: Triage duplicate local: emulates undefined
...
{
  "code": "DuplicateLocal",
  "message": "duplicate local binding: `$262` at 1426..1440",
  "span_start": 1426,
  "span_end": 1440,
  "line": 64,
  "column": 1,
...
```text
61 |   1. If Type(x) is different from Type(y), return false.
62 | features: [IsHTMLDDA]
63 | ---*/
64 | 
65 | var IsHTMLDDA = $262.IsHTMLDDA;
66 | 
67 | assert.sameValue(Object.is(IsHTMLDDA, undefined), false, "SameValue with `undefined`");
```
...
```text
error: [DuplicateLocal] duplicate local binding: `$262` at 1426..1440
```
...
```json
{
    ...
    "diagnostics": [
      {
        "code": 6504,
        "category": "Error",
        "message": "File '/tmp/tmpucfagb_p/test262-triage-node-input.js' is a JavaScript file. Did you mean to enable the 'allowJs' option?\n  The file is in the program because:\n    Root file specified for compilation"
      }
    ],
    "hints": [],
  ...
  "ast": {
    ...
    "pathToPosition": [
      ...
      {
        "kind": "ThrowStatement",
        "text": "throw new Test262Error(message);",
        "line": 59,
        "character": 3
      },
      {
        "kind": "NewExpression",
        "text": "new Test262Error(message)",
        "line": 59,
        "character": 9
      },
      {
