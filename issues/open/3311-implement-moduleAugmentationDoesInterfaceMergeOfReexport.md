---
id: 3311
title: "Implement Moduleaugmentationdoesinterfacemergeofreexport"
type: spike
...
namespace import, and use-site expressions. Module graph validation then fails
because `export * from "./file"` in virtual `reexport.ts` cannot resolve to
the sibling virtual `file.ts` section.

## Problem

`moduleAugmentationDoesInterfaceMergeOfReexport.ts` is a multi-section
...
static re-export source specifier, not interface-merge semantics.
...
```sh
env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/moduleAugmentationDoesInterfaceMergeOfReexport --detail --no-dashboard-data
```

Representative triage:

```sh
env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/moduleAugmentationDoesInterfaceMergeOfReexport.ts
```
...
`issues/open/5229a-resolve-imports-between-filename-sections.md` before
re-triaging interface merge behavior.
...
- Interface merge or module augmentation semantics.
...
- [x] Existing owner issue 5229 contains the exact `moduleAugmentationDoesInterfaceMergeOfReexport.ts` evidence.
...
- `reference/typescript/tests/cases/compiler/moduleAugmentationDoesInterfaceMergeOfReexport.ts`
...
```text
command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/moduleAugmentationDoesInterfaceMergeOfReexport --detail --no-dashboard-data
result: pass; executed=1, unsupported=1, UnsupportedModule:1
date: 2026-05-08

command: env TS2WASM_BINARY=/tmp/ts2wasm-issue-blockers-target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/moduleAugmentationDoesInterfaceMergeOfReexport.ts
...
- After issue 5229 advances, interface merge, module augmentation, and
