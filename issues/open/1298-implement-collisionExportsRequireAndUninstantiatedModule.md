---
id: 1298
title: "Implement Collisionexportsrequireanduninstantiatedmodule"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1298.

## Summary

Closed as stale. Fresh triage and focused coverage show
`reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts`
...
`collisionExportsRequireAndUninstantiatedModule` with diagnostics:
...
TS2322 nullability diagnostics, but semantic parity is not enabled for this
coverage window and those diagnostics are not the generated compiler build
blocker.
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts --detail --no-dashboard-data
```
...
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts --detail
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts
```
...
### Smart triage: Build pass: collisionExportsRequireAndUninstantiatedModule

- Issue class: `none`
- Feature label: `build-pass`
- Diagnostic: `BuildPass` / `pass`
- Path: `reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts`
...
```text
tokens: ok; includes export namespace require/exports, exported interfaces, and exported functions foo/foo2
...
visible symbols: functions foo and foo2
```
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts
...
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndUninstantiatedModule.ts --detail --no-dashboard-data
...
- TypeScript still reports TS2322 diagnostics; those are future semantic
