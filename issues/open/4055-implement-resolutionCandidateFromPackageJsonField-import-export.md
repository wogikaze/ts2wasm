---
id: 4055
title: "Implement Resolutioncandidatefrompackagejsonfield Import Export"
type: spike
...
Triage resolutionCandidateFromPackageJsonField-import-export across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `resolutionCandidateFromPackageJsonField-import-export` with diagnostics: import-export. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: resolutionCandidateFromPackageJsonField-import-export has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/resolutionCandidateFromPackageJsonField1.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/resolutionCandidateFromPackageJsonField1.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/resolutionCandidateFromPackageJsonField1.ts`
