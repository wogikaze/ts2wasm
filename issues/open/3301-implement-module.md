...
Triage module across 4 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 4 cases fail in directory `module` with diagnostics: import-export. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: module has 4 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/module_augmentExistingVariable.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/module_augmentExistingVariable.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/module_augmentExistingVariable.ts`
- `reference/typescript/tests/cases/compiler/module_augmentUninstantiatedModule.ts`
- `reference/typescript/tests/cases/compiler/module_augmentExistingAmbientVariable.ts`
- `reference/typescript/tests/cases/compiler/module_augmentUninstantiatedModule2.ts`
