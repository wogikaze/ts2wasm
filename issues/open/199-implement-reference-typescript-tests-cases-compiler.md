...
Close the generated `reference/typescript/tests/cases/compiler` bucket as stale: fresh triage shows `2dArrays.ts` now builds successfully.

## Problem

Reference test results used to show 1 case failing in directory `reference/typescript/tests/cases/compiler` with diagnostics: parser-syntax. Fresh triage on 2026-05-07 shows the affected file now builds successfully and TypeScript reports no oracle diagnostics.

Problem: this generated bucket is stale. The current compiler advances through the old typed class-field parser blocker in `2dArrays.ts`.
...
- `issues/open/5022-implement-array-every-receiver.md` already recorded a
  later `2dArrays.ts` BuildPass after the old parser blocker was removed.
- Broad parser-syntax duplicates are no-match for current implementation work
  because focused triage now returns BuildPass.
...
- `issues/open/077-implement-InterfaceDeclaration.md` - Implement Interfacedeclaration (same feature label, same group key, title overlap)
...
```text
command:
env TS2WASM_BINARY=/home/wogikaze/wgkz/ts2wasm/target/debug/ts2wasm python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/2dArrays.ts

result:
BuildPass / pass; ts2wasm build succeeded

compiler evidence:
tokens: ok; class fields such as `isSunk: boolean = false` tokenize
ast: ok; ClassDecl Cell, ClassDecl Ship, and ClassDecl Board parse; typed
     property declarations are erased from runtime class bodies
resolved: ok; Board.allShipsSunk resolves to a MethodCall on this.ships.every
...
```text
command:
env TS2WASM_BINARY=/home/wogikaze/wgkz/ts2wasm/target/debug/ts2wasm python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/2dArrays.ts --detail --no-dashboard-data
result:
