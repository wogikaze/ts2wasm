...
Triage the generated reference bucket `Implement Accessorbodyintypecontext` before implementation. This issue records a failing reference case and must be split or superseded before any code change starts.

## Problem

Reference test results show 1 cases fail in directory `accessorBodyInTypeContext` with diagnostics: parser-syntax. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: generated reference bucket `Implement Accessorbodyintypecontext` fails with `parser-syntax` and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/accessorBodyInTypeContext.ts
```

Narrow coverage reproduction:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/accessorBodyInTypeContext.ts --detail
```

Representative path: `reference/typescript/tests/cases/compiler/accessorBodyInTypeContext.ts`
...
- `reference/typescript/tests/cases/compiler/accessorBodyInTypeContext.ts`
...
Evidence files:
- `issues/open/095-implement-accessorBodyInTypeContext.md`
