---
id: 3197
title: "Implement Longobjectinstantiationchain Parser Syntax"
type: spike
...
Triage longObjectInstantiationChain-parser-syntax across 2 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 2 cases fail in directory `longObjectInstantiationChain-parser-syntax` with diagnostics: parser-syntax. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: longObjectInstantiationChain-parser-syntax has 2 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/longObjectInstantiationChain1.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/longObjectInstantiationChain1.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/longObjectInstantiationChain1.ts`
- `reference/typescript/tests/cases/compiler/longObjectInstantiationChain3.ts`
