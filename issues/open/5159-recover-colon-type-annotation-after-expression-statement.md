...
- [ ] Add parser recovery or a source-spanned frontend diagnostic for `expr: type;` after an expression statement, with focused coverage for `this.foo: any;`.
- [ ] Preserve normal expression statement parsing and ASI behavior for valid statements.
- [ ] Re-run triage for both `bases.ts` and `autoLift2.ts` and confirm the current `UnsupportedSyntax` colon blocker is gone.
...
- [ ] A focused parser/frontend test covers `class B { constructor() { this.y: any; } }`.
- [ ] `python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/bases.ts` no longer reports `UnsupportedSyntax: expected Semicolon, got Some(Colon)`.
- [ ] `python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/autoLift2.ts` no longer reports `UnsupportedSyntax: expected Semicolon, got Some(Colon)`.
- [ ] Valid expression statements and ASI parser tests still pass.
