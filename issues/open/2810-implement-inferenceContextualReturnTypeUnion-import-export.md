---
id: 2810
title: "Implement Inferencecontextualreturntypeunion Import Export"
type: spike
...
Triage inferenceContextualReturnTypeUnion-import-export across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `inferenceContextualReturnTypeUnion-import-export` with diagnostics: import-export. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: inferenceContextualReturnTypeUnion-import-export has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/inferenceContextualReturnTypeUnion4.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/inferenceContextualReturnTypeUnion4.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/inferenceContextualReturnTypeUnion4.ts`
