---
id: 2361
title: "Implement Extendnonclasssymbol Name Resolution"
type: spike
...
Triage extendNonClassSymbol-name-resolution across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `extendNonClassSymbol-name-resolution` with diagnostics: name-resolution. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: extendNonClassSymbol-name-resolution has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/extendNonClassSymbol1.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/extendNonClassSymbol1.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/extendNonClassSymbol1.ts`
