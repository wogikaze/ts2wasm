...
- [ ] Detect BigInt literal object keys such as `{ 1n: 123 }` and report an issue-linked TS1539-compatible diagnostic.
- [ ] Detect computed BigInt literal object keys such as `{ [1n]: 456 }` and report a source-spanned TS2464-compatible diagnostic or explicit follow-up boundary.
- [ ] Preserve existing accepted identifier, string, and numeric object keys.
- [ ] Add focused parser/frontend regression coverage for BigInt literal object property names.
- [ ] Re-run representative triage and confirm the first generic parser key diagnostic is gone.
...
- [ ] `parse_program("({ 1n: 123 });")` fails with a source-spanned BigInt property-name diagnostic, not `expected identifier or string literal as object key`.
- [ ] `parse_program("({ [1n]: 456 });")` fails with a source-spanned computed property-name diagnostic or a documented follow-up issue.
- [ ] Existing identifier/string/numeric object key tests still pass.
- [ ] `python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/bigintPropertyName.ts` no longer reports the first generic object-key parser diagnostic.
