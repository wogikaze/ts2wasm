...
Triage stringTrim across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 cases fail in directory `stringTrim` with diagnostics: builtin-api. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: stringTrim has 1 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/stringTrim.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/stringTrim.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/stringTrim.ts`
