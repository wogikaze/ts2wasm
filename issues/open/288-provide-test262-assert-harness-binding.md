...
- [ ] Provide a supported test262 harness binding or lowering path for `assert` in admitted reference cases.
- [ ] Representative arguments-object cases compile far enough to produce semantic pass/fail instead of unresolved `assert`.
- [ ] The exact `unresolved function:`assert`` stderr bucket is reduced or removed.
