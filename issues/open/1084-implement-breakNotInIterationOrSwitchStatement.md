---
id: 1084
title: "Implement Breaknotiniterationorswitchstatement"
type: spike
...
Triage breakNotInIterationOrSwitchStatement across 1 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 1 case failing in directory `breakNotInIterationOrSwitchStatement` with diagnostics: switch. Fresh smart triage shows the compiler already parses `break;` and reports `break must be inside a loop or switch` at the standalone statement, while TypeScript reports TS1105 at the same span.

Problem: `breakNotInIterationOrSwitchStatement` is not a standalone implementation order; the current failure is an oracle-matching invalid-break diagnostic covered by issue 035 break/continue behavior.
...
```sh
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts --detail
```
...
- `reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts`
...
### Smart triage: switch: breakNotInIterationOrSwitchStatement1

- Issue class: `triage-needed`
- Feature label: `switch`
- Diagnostic: `UnsupportedSyntax` / `parser-or-frontend-unsupported`
- Path: `reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts`

Reproduction:

```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts
```
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/breakNotInIterationOrSwitchStatement1.ts
