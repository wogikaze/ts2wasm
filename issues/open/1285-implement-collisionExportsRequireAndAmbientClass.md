---
id: 1285
title: "Implement Collisionexportsrequireandambientclass"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1285.

## Summary

Closed as stale. Fresh triage and focused coverage show
`reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts`
...
`collisionExportsRequireAndAmbientClass` with diagnostics: import-export. Fresh
...
TS2395 merged declaration export/local diagnostics for ambient classes named
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts --detail --no-dashboard-data
```
...
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts --detail
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts
```
...
### Smart triage: Build pass: collisionExportsRequireAndAmbientClass

- Issue class: `none`
- Feature label: `build-pass`
- Diagnostic: `BuildPass` / `pass`
- Path: `reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts`
...
```text
tokens: ok; includes export declare class require/exports, declare namespace m1, namespace m2, and namespace m4
ast: ok; []
resolved: ok; []
visible symbols: class require/exports declarations in module and namespace scopes plus binding a initialized to 10
```
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts
...
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndAmbientClass.ts --detail --no-dashboard-data
