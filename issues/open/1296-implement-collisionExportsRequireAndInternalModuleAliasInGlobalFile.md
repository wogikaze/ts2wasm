---
id: 1296
title: "Implement Collisionexportsrequireandinternalmodulealiasinglobalfile"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1296.

## Summary

Closed as stale. Fresh triage and focused coverage show
`reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts`
...
`collisionExportsRequireAndInternalModuleAliasInGlobalFile` with diagnostics:
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts --detail --no-dashboard-data
```
...
mise run reference-coverage -- tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts --detail
mise run reference-triage -- tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts
```
...
### Smart triage: Build pass: collisionExportsRequireAndInternalModuleAliasInGlobalFile

- Issue class: `none`
- Feature label: `build-pass`
- Diagnostic: `BuildPass` / `pass`
- Path: `reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts`
...
```text
tokens: ok; includes namespace mOfGloalFile, export class c, import aliases exports/require, and new exports/new require
ast: ok; import aliases are erased to Undefined expressions, followed by New exports and New require
resolved: ok; New class_name exports and New class_name require
...
```text
command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts
...
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/collisionExportsRequireAndInternalModuleAliasInGlobalFile.ts --detail --no-dashboard-data
...
- Semantic runtime parity is not enabled for this reference window. The current
  AST erases import aliases before `new exports()` / `new require()`, but there
  is no current compiler build blocker or TypeScript oracle diagnostic to split
