...
- [ ] Avoid registering `{}` and `[]` synthetic binding-pattern display names as local bindings.
- [ ] Preserve duplicate-local checks for real names inside non-empty binding patterns.
- [ ] Add focused coverage for repeated `const {} = f();` and `const [] = f();`.
...
- [ ] A focused test accepts repeated `const {} = f();` without `DuplicateLocal`.
- [ ] A focused test accepts repeated `const [] = f();` without `DuplicateLocal`.
- [ ] Existing duplicate-local tests for repeated real identifiers still fail.
- [ ] `python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/bindingPatternCannotBeOnlyInferenceSource.ts` no longer reports `duplicate local binding: {}`.
