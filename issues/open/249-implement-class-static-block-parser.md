...
- [ ] Parse `ClassStaticBlock` as a class body element.
- [ ] Preserve statement-list spans inside the static block.
- [ ] Reject invalid static-block-only forms according to parser-owned early checks where practical.
- [ ] Add parser and CLI dump/diagnostic coverage for static blocks.
...
- [ ] `class C { static { console.log(1); } }` parses as a class static block or reports an explicit issue-linked unsupported semantic diagnostic after parsing.
- [ ] `static {}` does not get misparsed as a static method or static field.
- [ ] Static block statement spans are preserved in AST/dump diagnostics.
- [ ] A focused test262 class-elements static-block reference slice no longer fails due to missing parser classification.
...
- [ ] update language-reference docs if status changes

Current state:

- [ ] update `current-state.md` if static block support boundary changes

Follow-up issues:

- [ ] create runtime class-static-block semantics issue if this slice stops at parser classification
