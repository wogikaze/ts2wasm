...
- [ ] Implement remaining check: `top_level_statements` must not contain `LoweredStmt::Function`
- [ ] Audit `validate.rs` for any other missing invariants (e.g., contiguous LocalId across params+locals, top_level_locals contiguous from 0)
- [ ] Update `docs/13-ir-contracts.md` validate_lowered table: mark all checks as `実装済み`
- [ ] Add negative test for top_level_statements containing Function
...
- [ ] `validate_lowered` rejects `LoweredStmt::Function` in `top_level_statements` with `InvariantViolation`
- [ ] Negative test added that triggers this check
- [ ] `docs/13-ir-contracts.md` `validate_lowered` table has all 4 checks marked `実装済み`
