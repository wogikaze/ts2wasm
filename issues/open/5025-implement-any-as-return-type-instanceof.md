...
> Reason: Zero implementation commits. Created as a triage split and batch-closed without code. Batch audit `3f0bfdf18` stamped 311 issues as truly-done without individual verification.
> Evidence: `git log --oneline --all --grep=5025` shows only re-open and chore close commits — no feat/fix commit.
> Required to close: Implementation commit that fixes instanceof RHS constructor resolution through return types.
...
**Why this was false-done**: This issue was created in commit `f0d3d720` as a triage split from unknown-unsupported, alongside 5022 (Array.prototype.every receiver), 5023 (APISample_watcher arrow), and 5024 (anonterface new identifier). Unlike 5022 (which received a real fix in `d5b1b669`), this issue was moved to `issues/done/` in commit `6182c373` **without any implementation commits**. All acceptance criteria boxes were incorrectly checked despite zero code changes. The issue was `class: blocked` with `depends_on: [5004]`, but 5004 is a meta-tracking issue — this child work was never implemented.
