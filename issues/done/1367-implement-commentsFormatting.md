---
id: 1367
title: "Implement Commentsformatting"
type: spike
...
> Evidence: Empty completion evidence. No feat/fix commit for #1367.
...
`commentsFormatting` with diagnostics: import-export. Fresh focused coverage on
2026-05-07 reports `build_pass=1` and no unsupported diagnostics.

Problem: stale issue; `commentsFormatting.ts` no longer has a current compiler
blocker.
...
```sh
python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/commentsFormatting.ts
```

Coverage window:

```sh
python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/commentsFormatting.ts --detail --no-dashboard-data
```
...
- `reference/typescript/tests/cases/compiler/commentsFormatting.ts`
...
```text
### Smart triage: Build pass: commentsFormatting

- Issue class: none
- Feature label: build-pass
- Diagnostic: BuildPass / pass
- Path: reference/typescript/tests/cases/compiler/commentsFormatting.ts
```
...
```text
command: python scripts/manager.py reference-coverage tsc --path-filter reference/typescript/tests/cases/compiler/commentsFormatting.ts --detail --no-dashboard-data
result: build_pass=1, unsupported=0, blocked=0
date: 2026-05-07

command: python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/commentsFormatting.ts
