...
- Implementation commits: verified via `git log --oneline --all --grep=5299`
...
Computed object binding parameters in arrow functions are lowered correctly.

Commits:
- IR lowering handles computed keys in destructuring parameters

Validation:
```sh
echo 'const b = ({ ["key"]: renamed }) => renamed;' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
