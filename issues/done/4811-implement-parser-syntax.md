...
Triage parser-syntax feature across 18 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 18 cases fail with parser-syntax diagnostic. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: parser-syntax feature has 18 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- tsgo reference/typescript-go/testdata/tests/cases/compiler/checkInheritedProperty.ts
```

Coverage window:

```sh
mise run reference-coverage -- tsgo --path-filter reference/typescript-go/testdata/tests/cases/compiler/checkInheritedProperty.ts --detail
```
...
```sh
mise run reference-coverage -- tsgo --limit 36
mise run reference-coverage -- tsgo --path-filter reference/typescript-go/testdata/tests/cases/compiler/checkInheritedProperty.ts --detail
mise run reference-triage -- tsgo reference/typescript-go/testdata/tests/cases/compiler/checkInheritedProperty.ts
```
...
- `reference/typescript-go/testdata/tests/cases/compiler/duplicateIdentifierChecks.ts`
- `reference/typescript-go/testdata/tests/cases/compiler/invalidGlobalAugmentation.ts`
- `reference/typescript-go/testdata/tests/cases/compiler/isolatedDeclarationsAsConstNestedObjects.ts`
- `reference/typescript-go/testdata/tests/cases/compiler/mergedEnumDeclarationMemberReferences.ts`
- `reference/typescript-go/testdata/tests/cases/compiler/missingDiscriminants2.ts`
- `reference/typescript-go/testdata/tests/cases/compiler/missingDiscriminants.ts`
- ... and 8 more files
...
Evidence files:
- `issues/done/4811-implement-parser-syntax.md`
