...
- [x] Resolve constructor-body `new foo()` when `foo` is a class declaration in
      the same module scope, including later declarations.
- [x] Preserve existing TDZ/runtime behavior decisions for unsupported
      expression-valued class uses; do not broaden to arbitrary class values.
- [x] Add or update a focused regression for the `classOrderBug.ts` shape.
- [x] Re-run the representative triage and split any later runtime/semantic
      blocker separately.
...
result: 31 tests passed (including 2 new focused tests)
date: 2026-05-08
```

Remaining risks:

- `public baz: foo` type annotation field is silently dropped (pre-existing behavior)
- Runtime/semantic behavior for `new foo()` on later class binding is a separate concern
