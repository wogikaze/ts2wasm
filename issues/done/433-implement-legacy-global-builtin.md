...
Triage legacy-global-builtin feature across 18 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 18 cases fail with legacy-global-builtin diagnostic. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: legacy-global-builtin feature has 18 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-coverage -- test262 --limit 36
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/escape/argument_bigint.js --detail
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/escape/argument_bigint.js
```
...
- `issues/done/344-implement-legacy-global-builtin-bindings.md` - Implement legacy global builtin bindings (8 test262 cases) (same feature label, same group key, title overlap)
...
### Smart triage: Triage unknown unsupported: argument bigint
...
```json
{
  ...
  "message": "UnsupportedTest262Metadata/test262-metadata: test262 feature `BigInt` is not supported by this runner slice",
  "span_start": null,
  "span_end": null,
  "line": null,
  "column": null,
...
```text
error: [UnsupportedSyntax] UnsupportedTest262Metadata/test262-metadata: test262 feature `BigInt` is not supported by this runner slice
```
    ...
    "diagnostics": [
      {
        "code": 6504,
        "category": "Error",
        "message": "File '/tmp/tmpd_na51po/test262-triage-node-input.js' is a JavaScript file. Did you mean to enable the 'allowJs' option?\n  The file is in the program because:\n    Root file specified for compilation"
      }
    ],
    "hints": [],
...
Evidence files:
- `issues/done/433-implement-legacy-global-builtin.md`
