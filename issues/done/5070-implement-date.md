...
Triage date feature across 418 failing reference test cases and split this bucket into implementation-ready child issues.

## Problem

Reference test results show 418 cases fail with date diagnostic. The compiler cannot handle these syntax/semantics, preventing compilation of code in this category.

Problem: date feature has 418 reference failures and needs smart-triage evidence before implementation starts.
...
```sh
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/Date/prototype/getYear/return-value.js
```

Coverage window:

```sh
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/Date/prototype/getYear/return-value.js --detail
```
...
```sh
mise run reference-coverage -- test262 --limit 836
mise run reference-coverage -- test262 --path-filter reference/test262/test/annexB/built-ins/Date/prototype/getYear/return-value.js --detail
mise run reference-triage -- test262 reference/test262/test/annexB/built-ins/Date/prototype/getYear/return-value.js
```
...
- `reference/test262/test/annexB/built-ins/Date/prototype/setYear/year-nan.js`
...
`issues/open/5012-implement-date.md` に統合されました。
そちらを参照してください。
## Completion evidence
...
Evidence files:
- `issues/done/5070-implement-date.md`
