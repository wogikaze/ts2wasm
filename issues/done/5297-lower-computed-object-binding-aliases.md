...
Computed object binding aliases in declarations are lowered correctly.

Commits:
- IR lowering handles computed keys in destructuring declarations

Validation:
```sh
echo 'let foo = "bar"; let {[foo]: bar} = {bar: "bar"};' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
