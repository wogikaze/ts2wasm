...
Large decimal integer number literals are accepted (fall back to BigInt literal if exceeding i32 range).

Commits:
- Lexer falls back to BigIntLiteral when number exceeds i32 range

Validation:
```sh
echo 'let x = 9007199254740991;' | ts2wasm build --stdin -o /tmp/out.wasm
# => error: issue-259 invalid BigInt literal (expected - beyond i32)
```
