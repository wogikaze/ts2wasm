...
Stmt::EnumDecl implemented for plain enum declarations. Parser produces Stmt::EnumDecl, name resolver registers enum names.
...
Validation:
```sh
echo 'enum E { A, B }; let x: any = E' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0 (enum name resolves)
```
