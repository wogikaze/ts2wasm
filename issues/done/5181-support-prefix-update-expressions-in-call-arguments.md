...
- [x] Support `++i` as a call argument when `i` is a resolved local identifier.
- [x] Preserve existing for-loop update support from issue `268`.
- [x] Preserve the diagnostic for non-identifier update targets.
- [x] Add focused coverage for `let i = 0; use(++i);`.
- [x] Re-run representative triage and confirm the current issue-268 diagnostic at `use(++i)` is gone.
...
- [x] `let i = 0; use(++i);` no longer reports `issue-268`.
- [x] The supported `++i` expression mutates `i` before passing the resulting value to `use`.
- [x] Existing `for (let i = 0; i < n; ++i)` coverage from issue `268` still passes.
- [x] A non-identifier update target still reports an issue-linked unsupported diagnostic.
- [x] `python scripts/manager.py reference-triage tsc reference/typescript/tests/cases/compiler/blockScopedBindingsReassignedInLoop1.ts` no longer reports the current `issue-268` diagnostic at `use(++i)`.
