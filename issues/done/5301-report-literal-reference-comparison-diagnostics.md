...
Literal reference comparison diagnostics are reported for `{a:1} === {a:1}`.

Commits:
- Name resolver detects literal comparisons and reports issue-5301

Validation:
```sh
echo 'let x = {a:1} === {a:1};' | ts2wasm build --stdin -o /tmp/out.wasm
# => error: [UnsupportedSyntax] issue-5301
```
