...
- Implementation commits: verified via `git log --oneline --all --grep=5285`
...
Export var initializer declarations are handled correctly.

Commits:
- Compiler/lib.rs: rewrites `export var x = 1` as module export

Validation:
```sh
echo 'export var x = 1;' | ts2wasm build --stdin -o /tmp/out.wasm
# => exit 0
```
